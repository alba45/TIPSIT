...
//-------------------------------- codice Timer -----------------------//
  ...
  int sec = 0;
  bool started =
      true; //variabili che mi aiutano a capire se i tasti sono stati premuti
  bool stopped = true;
  int timefortimer =
      0; //il nostro tempo che convertiremo tramite un algoritmo che trasforma il tempo da secondi in ore minuti e secondi
  bool checktimer = true;
  String timetodisplay = "00:00:00";

  void start() {
    //per l'avvio del timer imposto i valori booleani
    setState(() {
      started = false;
      stopped = false;
    });

    timefortimer = ((hour * 60 * 60) +
        (min * 60) +
        sec); //trasformo il tempo preso dai NumberPicker
    Timer.periodic(
        Duration(
          //imposto il timer e la durata
          seconds: 1,
        ), (Timer t) {
      setState(() {
        if (timefortimer < 1 || checktimer == false) {
          //controllo se è finito il tempo o se abbiamo premuto stop
          t.cancel();
          checktimer = true;
...
//--------------------------------------------------------------//
          ...
          //da qua parte il logaritmo che trasforma il mio tempo
          if (timefortimer < 10) {
            //nel formato stringa che voglio avere per stamparlo sul'app
            timetodisplay =
                "00:00:0" + timefortimer.toString(); //finisce a riga 118
          } else {
  ...
  void stop() {
    //impostazioni booleani che mi fanno uscire dallo start
    setState(() {
      started = true;
      stopped = true;
      checktimer = false;
  ...
  Widget countdown() {
    //da riga 128 a riga 279 si sviluppa la mia interfaccia del timer
    return Container(
        child: Column(
      ...
      children: <Widget>[
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: <Widget>[
                  //impostazioni per l'interfaccia del NumberPicker
                  Padding(
                    padding: EdgeInsets.only(
                      bottom: 10.0,
                    ),
        ...
        Expanded(
          child: Text(
            timetodisplay,
            style: TextStyle(
              //qui visualizzo il mio timer che cambia
              ...
              children: <Widget>[
                RaisedButton(
                  ...
                  shape: RoundedRectangleBorder(
                    //i tasti quando vengono cliccati vengono disabilitati
                    borderRadius: BorderRadius.circular(
                        15.0), //sia lo start che lo stop/reset(perchè alla fine resetta)
                  ),
                ),
                RaisedButton(
  ...
  bool stopflag =
      false; //i flag mi indicano se i tasti reset o stop sono stati premuti
  ...
  void stopCronometro() {
    //queste tre funzioni "giocano" con i valori booleani per l'uso corretto dei tasti
    setState(() {
      stopispressed = true;
  ...
  Stream<int> stopWatchStream() {
    ...
    void resetTimer() {
      //quando resetto  cancello il timer
      if (timer != null) {
        timer.cancel();
        timer = null;
        counter = 0;
    ...
    void tick(_) {
      //il mio tick che va avanti finchè non viene premuto stop e succesivamente reset
      if (!stopflag) {
        while (stopflag == true) {}
  ...
  Widget cronometrostream() {
    //l'interfaccia per il cronometro (finisce a riga 471)
    return Container(
      child: Column(
        children: <Widget>[
          Expanded(
                  ...
                  RaisedButton(
                    onPressed: startispressed
                        ...
                        : () {
                            ...
                            startCronometro();
                            timerStream =
                                stopWatchStream(); //dentro ai bttoni si può vedere come il mio codice si svillupa
...
//------------------------------ Layout ---------------------------//
  ...
  @override
  void initState() {
    //Qui ho messo le impostazioni del Tabcontroller
    tb = TabController(
      length: 2,
      vsync: this,
    );
